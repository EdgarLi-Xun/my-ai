package cn.edgarli.infrastructure.observability;
 ...
 * ADR 0004 §5 / §11: trace_id injection + HTTP access log.
 * <p>
 * 路径白名单：只对 {@code /api/**} 生效（其他静态资源 / H2 console / favicon 不记）。
 * Path whitelist: only handles {@code /api/**} (static assets / H2 console / favicon are skipped).
 * <ol>
 ...
 * </ol>
 * 排序：注册到 {@link cn.edgarli.infrastructure.config.FilterConfig}，跑在 Spring Security 之前，
 * 因此 {@code finally} 阶段 MDC user_id 已被 {@link cn.edgarli.infrastructure.security.JwtAuthenticationFilter}
 * 注入。
 * Ordered in {@link cn.edgarli.infrastructure.config.FilterConfig} before Spring Security, so by
 * the {@code finally} stage {@link cn.edgarli.infrastructure.security.JwtAuthenticationFilter} has
 * already populated MDC user_id.
 */
@Component
public class TraceIdFilter implements Filter {
    ...
    /** Dedicated access logger; logback-spring.xml binds it to ./logs/access.jsonl. */
    ...
    /** Matches /api/conversations/{id}/... or /api/messages/{id}/... to extract IDs into MDC. */
    ...
    /**
     * 核心过滤逻辑：注入 trace_id、记录访问日志、清理 MDC。
     * Core filter logic: inject trace_id, record access log, clean MDC.
     *
     * @param servletRequest 请求 / servlet request
     * @param servletResponse 响应 / servlet response
    ...
    public void doFilter(ServletRequest servletRequest,
        ...
        // whitelist: only /api/** goes through trace + access log
        ...
        // try to extract conversation_id / message_id from path into MDC (useful for debugging)
        ...
        long start = System.currentTimeMillis(); // 请求开始时间戳 / request-start timestamp
        ...
        try {
            ...
            // read SecurityContext in finally: JwtAuthenticationFilter has populated it by now
     ...
     * Local single-machine deployments effectively use remoteAddr; X-Forwarded-For is reserved
     * for future reverse-proxy deployments.
     *
